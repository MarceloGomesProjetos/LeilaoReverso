...
exports.closeAuction = async (req, res) => {
    ...
    const buyerId = req.user.id; // O ID do comprador vem do middleware de autenticação

    try {
        // A lógica complexa foi movida para o auctionService
        const result = await auctionService.closeAuctionById(auctionId, buyerId);
        res.json(result);
    } catch (error) {
        console.error(`Erro ao fechar leilão ${auctionId} pelo usuário ${buyerId}:`, error);
